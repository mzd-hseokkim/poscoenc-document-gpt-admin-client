...
import { CBadge, CCard, CCardBody, CCardHeader, CCol, CModalBody, CModalFooter, CRow } from '@coreui/react-pro';
...
const DocumentChatHistoryDetailForm = ({ initialFormMode, closeModal, refreshDocumentCollectionList }) => {
  ...
  const fetchChatHistoryDetail = useCallback(async () => {
    if (!chatIdParam) {
      return;
    }
    setIsLoading(true);
    try {
      const detail = await DocumentChatHistoryService.getDocumentChatHistory(chatIdParam);
      ...
      if (error.response?.status === 404) {
        addToast({ message: `id={${chatIdParam}} 해당 대화 이력을 찾을 수 없습니다.` });
      } else {
        console.log(error);
      }
      closeModal();
  ...
  }, [addToast, chatIdParam, closeModal, reset]);

  useEffect(() => {
    setIsLoading(false);
    void fetchChatHistoryDetail();
  }, [chatIdParam, fetchChatHistoryDetail, searchParams]);
    ...
    <>
      ...
      <CModalBody>
        ...
        <CCard className="mb-3">
          ...
          <CCardBody>
            <FormInputGrid fields={chatHistoryAttributeFields} formData={chatHistory} isReadMode={isReadMode} col={2} />
          </CCardBody>
        </CCard>
